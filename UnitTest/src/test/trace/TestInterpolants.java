...
public class TestInterpolants {
	@BeforeAll
	static void init() {
	}

	@Test
	void computeSimpleTrace() {
		// x:=0 y:=0 x++ x==-1
		...
		TraceScriptToInterpolants interpol = new TraceScriptToInterpolants(result, 4);
		Term[] interpolTerms = interpol.getResult();
		for (Term term : interpolTerms) {
			System.out.println(term);
		}
		System.out.println();
	}
	...
	@Test
	void computeComplexSingleVariableTrace() {
		...
		TraceScriptToInterpolants interpol = new TraceScriptToInterpolants(result, 6);
		Term[] interpolTerms = interpol.getResult();
		for (Term term : interpolTerms) {
			System.out.println(term);
		}
	...
	@Test
	void computeComplexMultiVariable() {
		...
		TraceScriptToInterpolants interpol = new TraceScriptToInterpolants(result, 8);
		Term[] interpolTerms = interpol.getResult();
		for (Term term : interpolTerms) {
			System.out.println(term);
		}
