...
public class TraceGeneralization {
	...
	public TraceGeneralization(Set<IPredicate> interpolants, Set<IStatement> allStatements,
			TraceToInterpolants interpolator, BasicPredicateFactory pf) {
		this.mInterpolants = interpolants;
		this.mAllStatements = allStatements;
		this.mInterpolator = interpolator;

		this.mInterpolants.add(interpolator.getTruePredicate());
		this.mInterpolants.add(interpolator.getFalsePredicate());
	...
	private void computeResult() {
		...
		this.mInterpolantNfa = new NestedWordAutomaton<IStatement, IPredicate>(automataService, alphabet,
				new GeneralizeStateFactory(pf));
		...
		for (IPredicate pre : this.mInterpolants) {
			for (IStatement statement : this.mAllStatements) {
				for (IPredicate post : this.mInterpolants) {
					...
					if (unsat) {
						if (DEBUG) {
							String trans = "(" + pre.getFormula().toString() + ", " + statement.toString() + ", "
									+ post.getFormula().toString() + ")";
						...
						if (!states.contains(pre))
							this.mInterpolantNfa.addState(pre.equals(this.mInterpolator.getTruePredicate()),
									pre.equals(this.mInterpolator.getFalsePredicate()), pre);
						if (!states.contains(post))
							this.mInterpolantNfa.addState(post.equals(this.mInterpolator.getTruePredicate()),
									post.equals(this.mInterpolator.getFalsePredicate()), post);
