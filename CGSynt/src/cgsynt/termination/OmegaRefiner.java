...
public class OmegaRefiner {
	...
	public OmegaRefiner(TraceGlobalVariables globalVars) {
		...
		mPredicateFactory = new PredicateFactory(mServiceProvider, mMScript,
				mRankVarConstructor.getCsToolkitWithRankVariables().getSymbolTable(), SimplificationTechnique.NONE,
				XnfConversionTechnique.BDD_BASED);

		mCsToolkitWithRankVars = mRankVarConstructor.getCsToolkitWithRankVariables();

		mBspm = new BinaryStatePredicateManager(mCsToolkitWithRankVars, mPredicateFactory,
				mRankVarConstructor.getUnseededVariable(), mRankVarConstructor.getOldRankVariables(), mServiceProvider,
	...
	public NestedWordAutomaton<IcfgInternalTransition, IPredicate> certifyCE(
		...
		NestedWord<IcfgInternalTransition> stemWord = new NestedWord<IcfgInternalTransition>(stemTransitions,
				stemNestingRelation);
		...
		NestedWord<IcfgInternalTransition> loopWord = new NestedWord<IcfgInternalTransition>(loopTransitions,
				loopNestingRelation);
		...
		LassoCheck<IcfgInternalTransition> check = new LassoCheck<>(InterpolationTechnique.Craig_NestedInterpolation,
		...
		// return refiner.refineBuchi(omega, counterexample,
		// curNumOmegaRefinementIterations, setting, bspm, modifiableGlobalsTable,
		// interpolation, benchmarkGenerator, complementationConstruction);
		return null;
	}
	...
	private RefinementStrategyFactory<IcfgInternalTransition> setUpRefinementFactory(BasicIcfg<IcfgLocation> icfg) {
		...
		CegarAbsIntRunner<IcfgInternalTransition> absIntRunner = new CegarAbsIntRunner<>(mServiceProvider, mBenchmarker,
				icfg, SimplificationTechnique.NONE, XnfConversionTechnique.BDD_BASED, mTTI.getCfgSmtToolkit(),
				pathProgramCache, taPrefs);

		PredicateFactoryForInterpolantAutomata stateFactory = new PredicateFactoryForInterpolantAutomata(
				mCsToolkitWithRankVars.getManagedScript(), mPredicateFactory, taPrefs.computeHoareAnnotation());

		InterpolantAutomatonBuilderFactory<IcfgInternalTransition> interpolantAutomatonBuilderFactory = new InterpolantAutomatonBuilderFactory<>(
				mServiceProvider, mTTI.getCfgSmtToolkit(), stateFactory, icfg, absIntRunner, taPrefs,
				InterpolationTechnique.Craig_NestedInterpolation, taPrefs.interpolantAutomaton(), mBenchmarker);

		TaCheckAndRefinementPreferences<IcfgInternalTransition> taCheckAndRefinementPrefs = new TaCheckAndRefinementPreferences<>(
				mServiceProvider, taPrefs, InterpolationTechnique.Craig_NestedInterpolation,
				SimplificationTechnique.NONE, XnfConversionTechnique.BDD_BASED, mTTI.getCfgSmtToolkit(),
				mPredicateFactory, icfg, interpolantAutomatonBuilderFactory);

		RefinementStrategyFactory<IcfgInternalTransition> refinementFactory = new RefinementStrategyFactory<>(mLogger,
				mServiceProvider, taPrefs, taCheckAndRefinementPrefs, absIntRunner, icfg, mPredicateFactory,
				pathProgramCache);

		return refinementFactory;
	}

	private TransitionStatePackage[] getTransitionStatePackages(
			DfaParityCounterexample<IcfgInternalTransition, IPredicate, IParityState> trace,
			BasicIcfg<IcfgLocation> icfg) {
		...
		HoareAnnotation startAnnot = mOldPredicateFactory.getNewHoareAnnotation(prevLocation,
		...
		while (!trace.loopStates.empty()) {
			...
			IcfgLocation curLocation = new IcfgLocation(
					new StringDebugIdentifier("0"/* "p1l" + (stemCount + loopCount) */), "p1");
